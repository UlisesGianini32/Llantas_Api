<x-layouts.app title="Editar llanta">
    ...
    <div class="mx-auto max-w-lg rounded-lg border border-neutral-800 bg-neutral-900 p-6">
        ...
        <h1 class="mb-6 text-xl font-bold text-white">
            ✏️ Editar llanta
        </h1>
        ...
        <form method="POST" action="{{ route('llantas.update', $llanta->id) }}">
            ...
            {{-- MARCA --}}
            <div class="mb-4">
                <label class="block text-sm text-gray-400">Marca</label>
                <input type="text" name="marca"
                       value="{{ old('marca', $llanta->marca) }}"
                       required
                       class="w-full rounded bg-neutral-800 border border-neutral-700 p-2 text-white">
            </div>

            {{-- MEDIDA --}}
            <div class="mb-4">
                <label class="block text-sm text-gray-400">Medida</label>
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" rows="3"
                          class="w-full rounded bg-neutral-800 border border-neutral-700 p-2 text-white">{{ old('descripcion', $llanta->descripcion) }}</textarea>
            </div>

            {{-- TÍTULO --}}
            <div class="mb-4">
                <label class="block text-sm text-gray-400">Título</label>
            ...
            {{-- PRECIO ML --}}
            <div class="mb-4">
                <label class="block text-sm text-gray-400">Precio ML</label>
            ...
            {{-- STOCK --}}
            <div class="mb-6">
                <label class="block text-sm text-gray-400">Existencias</label>
